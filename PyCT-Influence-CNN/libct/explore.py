...
class ExplorationEngine:
    ...
    def _execution_loop(self, max_iterations: int, all_args, concolic_dict):
        ...
        while cont and (max_iterations == 0 or iterations < max_iterations):
            ...
            while len(self.constraints_to_solve) > 0:
                constraint: Constraint = self.pop_constraint()
    ...
    def explore(
            ...
            model_path=None, input_for_shap=None, background_dataset_for_shap=None,
    ...
    def get_shap_value(self, position):
        layer_number, indices = position
        return self.comparator.get_shap_value(layer_number, indices)

    def push_constraint(self, constraint: Constraint, position):
        if self.constraints_collection_type == 'priority_queue':
            shap_value = self.get_shap_value(position)
            heapq.heappush(self.constraints_to_solve, (-abs(shap_value), constraint.id, constraint))
        else:
    ...
    def pop_constraint(self) -> Constraint:
        if self.constraints_collection_type =='stack':
            ...
            shap_value, constraint_id, constraint = heapq.heappop(self.constraints_to_solve)
            return constraint