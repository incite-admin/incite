...
class ShapValuesComparator:
    def __init__(
        ...
        background_dataset,
        input,
        explainer_type: Literal["deep", "kernel", "calculated"] = "deep",
    ) -> None:
        self.model = model
        self.background_dataset = background_dataset
        self.input = input
        self.shap_values: dict[str, float] = dict()
        self.explainer_type = explainer_type  # : Literal["deep", "kernel"]
    ...
    def compare(
        ...
        # if row_number_1 < row_number_2: return 1
        return self.get_shap_value(row_number_1, indices_1) - self.get_shap_value(
    ...
    def calculate_shap_values(
        ...
        if self.explainer_type == "deep":
        ...
        elif self.explainer_type == "calculated":
            shap_values = np.load("./shap_value/mnist_rnn_09650/mnist_shap_value.npy")
            shap_values = shap_values[0]
            for indices, shap_value in np.ndenumerate(shap_values):
         
                # print("------indices------", indices)
                # print("layer_number - 1", layer_number - 1)
                # print("------key------",  ShapValuesComparator.get_position_key(layer_number - 1, indices))
                self.shap_values[
                    ShapValuesComparator.get_position_key(layer_number - 1, indices)
                ] = shap_value
            ...
            if shouldFlattenInputToUseKernelExplainer:
                ...
                print('!!!!!!!!!!!!!')
                model.summary()
                # model.save('model/reshaped_lstm.h5')
                # np.save('model/reshaped_lstm_background.npy', background_dataset)
                # np.save('model/reshaped_lstm_input.npy', input)
...
class ShapValuesComparatorTester:
    ...
    def test_get_shap_value(self):
        ...
        assert (
            self.comparator.get_shap_value(1, (0, 0, 0))
            == self.comparator.shap_values["1_0_0_0"]
        )
        assert (
            self.comparator.get_shap_value(1, (0, 0, 31))
